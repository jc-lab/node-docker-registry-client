...
function PassThroughWithDigest(options) {
    streams.Transform.call(this, options);
    this._hash = options.hash;
    this._readerPaused = false;
    this._queuedChunk = null;
    this._digest = null;
    this._processedBytes = 0;
    this._super = streams.Transform.prototype;
}
PassThroughWithDigest.create = function (source, opts) {
    const instance = new PassThroughWithDigest(opts);
    return new Proxy(instance, {
        get: function (oTarget, sKey) {
            return (sKey in oTarget) ? oTarget[sKey] : source[sKey];
        },
        ...
        has: function (oTarget, sKey) {
            return sKey in source;
        },
        defineProperty: function (oTarget, sKey, oDesc) {
            return Object.defineProperty(source, sKey, oDesc);
        },
        getOwnPropertyDescriptor: function (oTarget, sKey) {
            return Object.getOwnPropertyDescriptor(source, sKey);
        },
    });
}
PassThroughWithDigest.prototype._transform = function (chunk, encoding, callback) {
    if (this._hash) {
        this._hash.update(chunk);
    }
    this._processedBytes += Buffer.isBuffer(chunk) ? chunk.byteLength : chunk.length;
    if (this._readerPaused) {
        this._queuedChunk = {
...
PassThroughWithDigest.prototype._read = function (size) {
    this._readerPaused = false;
    if (this._queuedChunk) {
        if (!this.push(this._queuedChunk.chunk, this._queuedChunk.encoding)) {
            this._readerPaused = true;
        }
        this._queuedChunk.callback();
        this._queuedChunk = null;
    }
    this._super._read.call(this, size);
}
PassThroughWithDigest.prototype._flush = function (callback) {
    if (this._hash) {
        this._digest = this._hash.digest();
    }
    if (this._super._flush) this._super._flush.call(this, callback);
    else callback();
}
PassThroughWithDigest.prototype.getDigest = function () {
    return this._digest;
}
PassThroughWithDigest.prototype.getProcessedBytes = function () {
    return this._processedBytes;
}
util.inherits(PassThroughWithDigest, streams.Transform);
...
module.exports = {
    ...
    PassThroughWithDigest: PassThroughWithDigest
};

...
RegistryClientV2.prototype.createBlobReadStream =
    ...
    this._headOrGetBlob({
        ...
        stream = stream.pipe(common.PassThroughWithDigest.create(stream, {
            hash: dcdInfo && dcdInfo.hash
        }))
          .on('end', () => {
              var cLen = Number(stream.headers['content-length']);
              var numBytes = stream.getProcessedBytes();
              if (!isNaN(cLen) && numBytes !== cLen) {
                  ...
                  var digest = stream.getDigest().toString('hex');
                  if (dcdInfo.expectedDigest !== digest) {
